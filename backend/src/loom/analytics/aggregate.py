...
from loom.schemas.response import AnalyticsResult, RankedCount
...
def merge_analytics(results: list[AnalyticsResult]) -> AnalyticsResult:
    ...
    the result is identical to what compute_analytics would have produced had
    every ticket from every analysis been processed together in one batch.
    """
    ...
    return AnalyticsResult(
        ...
        top_categories=top_categories,
        top_themes=top_themes,
        processing_success_rate=_pct(total_processed, total_processed + total_skipped),
        positive_pct=_pct(sentiment_counts.get("Positive", 0), total_processed),
        neutral_pct=_pct(sentiment_counts.get("Neutral", 0), total_processed),
        negative_pct=_pct(sentiment_counts.get("Negative", 0), total_processed),
        average_sentiment_score=average_sentiment_score,
        high_urgency_count=urgency_counts.get("High", 0),
    )

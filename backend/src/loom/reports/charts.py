...
from loom.schemas.taxonomy import Category, Sentiment, Urgency
...
def _horizontal_bar_chart(labels: list[str], values: list[int], bar_colors: list[str]) -> bytes:
...
def category_distribution_chart(top_categories: list[dict]) -> bytes:
    ...
    return _horizontal_bar_chart(labels, values, colors)


def theme_frequency_chart(top_themes: list[dict], top_n: int = 8) -> bytes:
    shown = top_themes[:top_n]
    labels = [t["name"] for t in shown]
    values = [t["count"] for t in shown]
    return _horizontal_bar_chart(labels, values, [THEME_COLOR] * len(labels))


def urgency_chart(urgency_distribution: dict[str, int]) -> bytes:
...
def sentiment_chart(positive_pct: float, neutral_pct: float, negative_pct: float) -> bytes:

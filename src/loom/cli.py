...
class LoomCLI:
    """Runs the exact same pipeline as the API, printed to the terminal."""
        ...
        for idx, text in enumerate(tickets):
            ...
            rows.append(
                RowRecord(
                    ...
                    source="cli-sample",
                    date=None,
                    word_count=TextInspector.word_count(text),
                )
            )
    ...
    def _print_items_table(self, items: list[TicketClassification]) -> None:
        ...
        for item in items:
            ...
            sentiment_cell = (
                f"[{sentiment_style}]{item.sentiment.value} "
                f"({item.sentiment_score:+.2f})[/{sentiment_style}]"
            )
    ...
    def _print_analytics(self, analytics: AnalyticsResult) -> None:
    ...
    @staticmethod
    def _text_rows(text: str) -> tuple[list[RowRecord], int, int]:
        row = RowRecord(
            ticket_id="0",
            raw_feedback=text,
            source="cli-text",
    ...
    async def run(self, csv_path: str | None, text: str | None, limit: int | None) -> None:
        self._print_banner()

        if csv_path:
            with self.console.status(f"[bold blue]Reading {csv_path}..."):
                df = pd.read_csv(csv_path)
                try:
                    validation = self.validation_service.validate_csv(df)
                except FileValidationError as exc:
                    self.console.print(f"[bold red]Validation failed ({exc.code}):[/bold red] {exc.message}")
                    return
            ...
            if not text.strip():
                self.console.print("[bold red]--text was empty or whitespace-only.[/bold red]")
                return
            rows, total_uploaded, skipped = self._text_rows(text)
            skip_reasons = {}
        else:
            rows, total_uploaded, skipped = self._sample_rows(limit)
            skip_reasons = {"empty_or_null_feedback": skipped} if skipped else {}

        with self.console.status(
            f"[bold blue]Classifying {len(rows)} tickets against '{config.LLM_MODEL}'..."
        ):
            items = await self.classification_service.classify_all(rows)
        ...
        analytics = self.analytics_service.compute(items, total_uploaded=total_uploaded, skipped=skipped)

        with self.console.status("[bold blue]Generating executive summary..."):
        ...
        self.console.print()
        self._print_analytics(analytics)
        self.console.print()
        self.console.print(Panel(summary, title="Executive Summary", border_style="blue", padding=(1, 2)))
        self.console.print()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--text", dest="text", default=None, help="Classify a single piece of feedback text directly"
    )
    parser.add_argument(
...
if __name__ == "__main__":
    args = parse_args()
    asyncio.run(LoomCLI().run(args.csv_path, args.text, args.limit))

...
Deterministic layout over data that already exists in AnalysisRepository.get_analysis:
...
this class only lays out numbers Python already produced elsewhere.
"""
...
from loom.services.charts import ChartRenderer
...
class ReportService:
    """Lays out one saved analysis into a printable weekly-report PDF."""
    ...
    def _styles(self) -> dict:
        ...
        return {
            ...
            "subtitle": ParagraphStyle("subtitle", parent=base["Normal"], fontSize=9.5, textColor=INK_MUTED),
            "h2": ParagraphStyle(
                "h2",
            ...
            "table_cell": ParagraphStyle("table_cell", fontSize=8.5, textColor=INK_SECONDARY, leading=11),
        }

    def _kpi_cell(self, label: str, value: str, tone, styles: dict) -> Table:
    ...
    def _kpi_grid(self, analytics: dict, validation_report: dict, styles: dict) -> Table:
        ...
        grid = Table(
            [[self._kpi_cell(label, value, tone, styles) for label, value, tone in row] for row in rows],
    ...
    def _chart_image(self, png_bytes: bytes, width_cm: float) -> Image:
    ...
    def _summary_box(self, summary: str, styles: dict) -> Table:
    ...
    def _highlighted_tickets_table(self, items: list[dict], styles: dict, limit: int = 10) -> Table:
        ranked = sorted(
            items, key=lambda t: (URGENCY_RANK.get(t["urgency"], 0), t["actionable"]), reverse=True
        )
        shown = ranked[:limit]
    ...
    def build_weekly_report_pdf(self, record: dict) -> bytes:
        """`record` is the dict returned by AnalysisRepository.get_analysis: analysis_id,
        ...
        story: list = [
            ...
            Spacer(1, 4),
            Paragraph(
                f"Analysis run on {self._format_date(record['created_at'])} &nbsp;&middot;&nbsp; "
            ...
            self._kpi_grid(analytics, validation_report, styles),
            KeepTogether(
                [Paragraph("Executive Summary", styles["h2"]), self._summary_box(record["summary"], styles)]
            ),
            KeepTogether(
                [
                    ...
                    self._chart_image(
                        self.chart_renderer.category_distribution_chart(analytics["top_categories"]),
                ...
                [
                    ...
                    Table(
                        [
                            ...
                            [
                                self._chart_image(
                                    self.chart_renderer.sentiment_chart(
                                ...
                                self._chart_image(
                                    self.chart_renderer.urgency_chart(analytics["urgency_distribution"]),
                                    width_cm=CONTENT_WIDTH / cm / 2 - 0.3,
                                ),
                            ],
                        ],
            ...
            KeepTogether(
                [
                    ...
                    self._chart_image(
                        self.chart_renderer.theme_frequency_chart(analytics["top_themes"]),
                        width_cm=CONTENT_WIDTH / cm,
                    ),
                ]
            ),
            ...
            self._highlighted_tickets_table(record["items"], styles),
        ]
